...
def read_from_mysqldb(sql_query, conf):
    try:
...
def save_to_mysqldb(db_table,operation,conf,input_df):
    try:
        # Connect to DB and store data in DB
        if operation == 'update':
            batchqr = input_df['batchqr'][0]
            oprvalid = input_df['oprvalid'][0]
            engine = start_engine(conf)
            que = "Update " + db_table + " set batchqr = " + "'" + str(batchqr) + "'" + ", oprvalid = " + "'" +\
                  str(oprvalid) + "'" + "where batchqr is null"
